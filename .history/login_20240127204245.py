...
def execution(): 
    ...
    if count > 0:
        ...
        if items:
            ...
            print(comment_check.text)
            if comment_check.text == '0': #ここから　　出品終了のタグを見つけて自動削除させる
                selling_exit = driver.find_element(By.CLASS_NAME,'item-row-edit')
                selling_exit_a = selling_exit.find_element(By.XPATH,'//*[@id="content-wrapper"]/div/div[2]/div/div[4]/div[1]/div[3]/a[1]')
                # l = selling_exit_a.get_attribute('href')
                selling_exit_a.click()
                
                alert = driver.switch_to.alert 
                alert.accept() #出品削除の警告文を自動OKにする
...
#20分おきにexecution()を実行
schedule.every(2).minutes.do(execution)
